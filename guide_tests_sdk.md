...
本文档提供 Parlant SDK 8 个核心特性的详细测试指导，包括测试目标、测试用例和操作步骤。
...
1. [环境准备](#环境准备)
2. [Agents（代理）](#agents代理)
3. [Guidelines（指导方针）](#guidelines指导方针)
4. [Journeys（旅程）](#journeys旅程)
5. [Tools（工具）](#tools工具)
6. [Glossary（领域词汇）](#glossary领域词汇)
7. [Canned Responses（预设回复）](#canned-responses预设回复)
8. [Context Variables（上下文变量）](#context-variables上下文变量)
9. [Sessions（会话管理）](#sessions会话管理)
10. [运行所有测试](#运行所有测试)
...
以`Test_that_an_agent_can_be_found_using_tool_context`作为例子，这是一个 **测试类**，用来测试 Agent 和 Tool 的结合使用。：

```python
class Test_that_an_agent_can_be_found_using_tool_context(SDKTest):
    async def setup(self, server: p.Server) -> None:  # setup: 在测试开始前执行的准备代码
        ...
        @p.tool  # 装饰器：告诉系统这是一个工具
        async def check_what_is_spatio(context: ToolContext) -> ToolResult:  # 定义了一个工具 check_what_is_spatio
            ...
            if agent is None:
                # 若找不到这个agent，则返回“spatio 是一种特殊的意面勺。”
                return ToolResult("A spatio is a special type of spaghetti spoon.")
            else:
                # 若找到了这个agent，则返回“spatio 是一只著名的虚构老鼠。”
...
- 这个功能让 Tool 能够知道自己在为哪个 Agent 工作
- 若 Tool 正确返回了答案，但测试结果为FAILED，则是因为 `nlp_test()` 硬编码使用 `GPT_4o`（OpenAI 模型），所以必须设置 `OPENAI_API_KEY`，NLP 验证步骤因缺少 API Key 而失败

**测试结论：**
- 功能价值：这个测试验证了 **Tool 可以访问它所属的 Agent 信息**
- 业务价值：同一个 **Tool 可以根据不同 Agent 返回不同结果**，好处是**不用为每个 Agent 重复写 Tool，减少维护成本**。比如：
    1. 多个门店共用一个"获取营业时间"工具，该工具可以根据不同门店返回不同营业时间。根据 Agent 角色调整回复
    2. 多个角色共用一个"产品查询"工具，该工具可以根据面向的不同角色返回不同的答复，比如给“技术支持”角色提供“保修信息”，给“销售顾问”角色提供“促销活动” 
...
以`Test_that_the_output_of_an_agent_can_be_intercepted`作为例子：

```python
class Test_that_the_output_of_an_agent_can_be_intercepted(SDKTest):
    # 注册一个"钩子"：在消息生成后、发送前拦截它
...
| 2. Agent 本想回复正常内容 | ✓ |
| 3. 钩子拦截并替换消息 | ✓ |
| 4. 最终返回 "Bananas! More bananas!" | ✓ 测试通过 |

**测试结论：**
- 功能价值：这个测试验证了 **"最后一公里"控制力** —— 在消息送到用户之前，你有最后一次机会修改、验证、或阻止它。
...
async def test_agent():
    async with p.Server() as server:
        ...
        # 查找 Agent
        found = await server.find_agent(id=agent.id)
        print(f"Agent found: {found.name}")
...
```python
# 蕴含关系
g1 = await agent.create_guideline(condition="Customer is upset", action="Transfer to manager")
g2 = await agent.create_guideline(condition="New customer arrives", action="Offer help")
await g1.entail(g2)  # g1 蕴含 g2

# 依赖关系
g1 = await agent.create_guideline(condition="Customer asks about price", action="Give price")
g2 = await agent.create_guideline(condition="Customer is frustrated", action="Apologize")
await g2.depend_on(g1)  # g2 依赖于 g1

# 消歧关系
g1 = await agent.create_guideline(condition="Customer is thirsty")
g2 = await agent.create_guideline(condition="Customer says hello")
g3 = await agent.create_guideline(condition="Customer asks about pizza")
await g1.disambiguate([g2, g3])  # 当 g1 匹配时，让用户选择
```
...
以`Test_that_guideline_can_take_priority_over_another_guideline`作为例子，这是一个测试 **Guideline 优先级** 的测试类：
...
| 4. 不包含 Coca-Cola 的推荐 | ✓ 测试通过 |

**功能价值**：这个测试验证了 **Guideline 优先级机制** —— 当多个 Guideline 条件都匹配时，系统能够根据优先级选择执行哪一个。
...
t_ask = await journey.initial_state.transition_to(
    chat_state="Ask if customer wants breakfast"
)
...
以`Test_that_scoped_guideline_of_matched_journey_without_states_influence_response`作为例子，这是一个测试 **旅程与 Guideline 结合** 的测试类：
...
class Test_that_scoped_guideline_of_matched_journey_without_states_influence_response(SDKTest):
 ...
 Journey",
            conditions            title="Test=["Customer greets you"],
...
| 3. 用户发送"Hello!"，旅程被激活 | ✓ |
| 4. Agent 返回包含"Pepsi"的响应 | ✓ 测试通过 |

**功能价值**：这个测试验证了 **旅程与 Guideline 的结合使用** —— 旅程可以封装一系列的 Guideline，在特定条件下激活并指导对话流程。
...
以`Test_that_a_tool_can_access_current_customer`作为例子，这是一个测试 **Tool 访问当前客户信息** 的测试类：
...
class Test_that_a_tool_can_access_current_customer(SDKTest):
    async def setup(self, server: p.Server) -> None:
        ...
        # 将 Tool 附加到 Agent，条件是"用户要求设置标志或触发工具"
...
| 5. Tool 正确返回了当前客户的 ID | ✓ 测试通过 |

**功能价值**：这个测试验证了 **Tool 可以访问当前会话中的客户信息** —— Tool 能够知道是谁在与它对话，从而提供个性化的服务。
...
以`Test_that_a_glossary_term_can_be_created`作为例子，这是一个测试 **Glossary 术语创建** 的测试类：

```python
class Test_that_a_glossary_term_can_be_created(SDKTest):
    async def setup(self, server: p.Server) -> None:
        # 创建一个用于测试 Glossary 的 Agent
        self.agent = await server.create_agent(
            name="Rel Agent",
...
| 3. 从存储中读取术语 | ✓ |
| 4. 验证术语的 name、description、synonyms 都正确 | ✓ 测试通过 |

**功能价值**：这个测试验证了 **Glossary 术语的创建和管理** —— Agent 可以定义专业术语及其同义词，确保在对话中正确理解和使用这些术语。
...
await agent.create_term(
    ...
    description="Unforeseeable circumstances that prevent someone from fulfilling a contract.",
)
```

---

## Canned Responses（预设回复）
...
以`Test_that_canned_response_with_field_dependency_is_excluded_when_field_unavailable`作为例子，这是一个测试 **Canned Response 字段依赖** 的测试类：
...
class Test_that_canned_response_with_field_dependency_is_excluded_when_field_unavailable(SDKTest):
    async def setup(self, server: p.Server) -> None:
        ...
        canrep_with_dependency = await self.agent.create_canned_response(
            ...
            field_dependencies=["order"],  # 依赖 "order" 字段
        )

        # Guideline 使用这个 Canned Response
        # 由于没有工具提供 "order" 字段，该响应会被排除
...
- 当依赖的字段不可用时，Canned Response 会被自动排除，避免生成不完整的回复
...
| 5. Agent 返回一个不包含 "order" 的回退响应 | ✓ 测试通过 |

**功能价值**：这个测试验证了 **Canned Response 字段依赖机制** —— 确保回复只在所需数据可用时才使用，避免生成不完整或不准确的响应。
...
以`Test_that_a_variable_value_can_be_set_for_a_customer`作为例子，这是一个测试 **Context Variable 客户级别值设置** 的测试类：

```python
class Test_that_a_variable_value_can_be_set_for_a_customer(SDKTest):
    async def setup(self, server: p.Server) -> None:
        # 创建一个用于测试 Context Variable 的 Agent
        self.agent = await server.create_agent(
            name="Rel Agent",
...
| 4. 获取该客户的变量值，返回 "premium" | ✓ 测试通过 |

**功能价值**：这个测试验证了 **Context Variable 的客户级别值管理** —— 可以为不同客户设置个性化的变量值，实现千人千面的对话体验。
...
await subscription_var.set_global_value("free")  # 默认值
...
以`Test_that_a_customer_can_be_found_by_name`作为例子，这是一个测试 **客户按名称查找** 的测试类：
...
**功能价值**：这个测试验证了 **客户查找和管理功能** —— 可以通过不同方式（名称、ID）查找和管理客户信息。
...
```bash
# 只运行需要引擎的测试
pytest tests/sdk/test_journeys.py -m engine -v
```
...
```bash
# 显示详细输出
pytest tests/sdk/test_agents.py -v -s

# 只显示失败信息
pytest tests/sdk/ --tb=short

# 每次测试后清理
pytest tests/sdk/ --cleanup
```
...
1. **先运行现有测试**：确保环境正确后再编写新测试
2. **使用 SDKTest 基类**：所有测试都应继承 `SDKTest`
3. **使用 `async def`**：测试方法必须是异步的
4. **使用 `await`**：所有 I/O 操作都需要 await
5. **清理测试数据**：测试之间尽量独立
6. **使用 `reuse_session=True`**：在同一会话中测试多轮对话
